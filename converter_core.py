...
def add_files():
    filetypes = [
        ("Tüm desteklenen", "*.png;*.jpg;*.jpeg;*.webp;*.bmp;*.pdf;*.mp3;*.wav;*.ogg;*.flac;*.mp4;*.avi;*.mov;*.mkv"),
...
def convert_audio(input_path, out_dir, target_format):
    """Ses dönüştürme (mp3, wav, ogg, flac vs.)"""
    base_name = os.path.splitext(os.path.basename(input_path))[0]
    out_path = safe_output_path(out_dir, base_name, target_format)
...
def convert_video(input_path, out_dir, target_format):
    """Video dönüştürme (mp4, avi, mov, mkv vs.)"""
    base_name = os.path.splitext(os.path.basename(input_path))[0]
    out_path = safe_output_path(out_dir, base_name, target_format)

    clip = VideoFileClip(input_path)
    # Basit format dönüştürme (ileri seviye: bitrate, çözünürlük vs. eklenebilir)
...
def start_conversion():
    ...
    try:
        if op == "Resim format dönüştür":
            ...
            for path in selected_files:
                try:
                    convert_video(path, output_dir, target_format)
                    success += 1
                except Exception as e:
                    print("Video dönüştürme hatası:", path, e)
                    errors += 1
...
root = tk.Tk()
root.title("Gelişmiş Dosya Converter")
...
# Resize + kalite ayarları (sadece resimler için anlamlı ama her zaman görünebilir)
